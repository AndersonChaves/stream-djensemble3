...
def distance_dtw(X1, X2):
    return dtw.distance(X1, X2)

def convert_matrix_of_time_series_to_list(data):
    data_transposed = np.transpose(data, (1, 2, 0))
    n, m, t = data_transposed.shape
    return data_transposed.reshape((n * m, t))    

def generate_labels_from_cluster_lists(cluster_list):
    n_points = 0
...
def silhouette_samples_slow(X, labels, metric='euclidean', **kwds):
    ...
    metric = distance_dtw # Changed here
    n = labels.shape[0]
    A = np.array([_intra_cluster_distance_slow(X[:100], labels[:100], metric, i)
                  for i in range(n)])
...
def _intra_cluster_distances_block(X, labels, metric, n_jobs=1, **kwds):
    ...
    """
    metric = distance_dtw # Alterado aqui
