...
class ClusteringThroughTime():
    ...
    def run(self):
        ...
        for t in range(start, end, step):
            ...
            if self.config["type"] == "dynamic":
                silhouette = self.perform_dynamic_clustering(t, step)
    ...
    def perform_dynamic_clustering(self, t, step):
        self.load_dataset(t, t+step)
        data = self.ds.read_all_data()
        clustering = StaticClustering(data, self.embedding_method)
        clustering.run()
        self.static_clustering.save_clustering_image(f"images/dynamic/{self.embedding_method}/w={self.window_size}", f"{t}-{t+step}")
